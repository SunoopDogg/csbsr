...
from model.utils.metrics.surface_distance.metrics import lookup_tables  # pylint: disable=relative-beyond-top-level
import numpy as np
...
def compute_robust_hausdorff(surface_distances, percent, outlier=np.inf):
    ...
    if len(distances_gt_to_pred) > 0:    # pylint: disable=g-explicit-length-test
        ...
        perc_distance_gt_to_pred = outlier
    ...
    if len(distances_pred_to_gt) > 0:    # pylint: disable=g-explicit-length-test
        ...
        perc_distance_pred_to_gt = outlier

    return max(perc_distance_gt_to_pred, perc_distance_pred_to_gt) #, outlier_detect
